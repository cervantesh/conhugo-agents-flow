...
7. GitHub Actions will publish a GHCR image for `main` and version tags
8. Confirm the package page shows the new version at `https://pypi.org/project/conhugo-agents-flow/`
9. Confirm the container package is visible under `ghcr.io/cervantesh/conhugo-agents-flow`
10. Use manual dispatch only when you need an out-of-band publish run, such as re-running `target=pypi`
...
- container publishing goes to GHCR through `container.yml` using `GITHUB_TOKEN` with `packages: write`
